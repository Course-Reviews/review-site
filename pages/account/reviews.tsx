...
const Course: React.FC = () => {
  ...
  return (
    <Container as={'main'} className={'pb-24 flex-grow'}>
      ...
      {reviews ? (
        reviews.sort((r1, r2) => toMilliseconds(r2.review.createdAt) - toMilliseconds(r1.review.createdAt)).map((r, i) => (
          <div className={'my-4 flex flex-col'} key={i}>
            <h2
              className={'text-xl font-semibold text-gray-700'}
            >{`Review for ${r.code} - ${r.title}`}</h2>
            <div className={'mb-6 text-gray-500 font-semibold'}>You posted {formatTime(new Date(r.review.createdAt))}</div>
            <Review
